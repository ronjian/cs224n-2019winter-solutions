...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens = enc_hiddens.permute(1, 0, 2) # (src_len, b, 2 * h) -> (b, src_len, h*2)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens) #  (b, src_len, h*2) X (h*2, h) -> (b, src_len, h)
        Y = self.model_embeddings.target(target_padded) # (tgt_len, b) -> (tgt_len, b, e)
        for Y_t in torch.split(tensor=Y
                               , split_size_or_sections=1
                               , dim=0):
            Y_t = Y_t.squeeze() # (1, b, e) -> (b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(torch.unsqueeze(dec_hidden, dim=1)
                        , enc_hiddens_proj.permute(0,2,1)) # (b, 1, h) X (b, h, src_len) -> (b, 1, src_len)
        e_t = torch.squeeze(e_t, dim=1) # (b, 1, src_len)-> (b, src_len)
        ### END YOUR CODE

        # Set e_t to -inf where enc_masks has 1 ('pad')
        ...
        # 2
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens), dim=1) # (b,1,src_len) X (b, src_len, 2h) -> (b,1,2h) -> (b,2h)
        # 3
        U_t = torch.cat((a_t, dec_hidden), 1) # (b, 2h), (b, h) -> (b, 3h)
        # 4
        V_t = self.combined_output_projection(U_t) # (b, 3h) X (3h, h) -> (b, h)
        # 5
        O_t = nn.Tanh()(V_t)
